...
foreach(@array){
    ...
    if (defined $meta{(split /\t/, $_)[0]}){
        print FINAL "$_\t", $meta{(split /\t/, $_)[0]}, "\t", "ST: ", $ST{(split /\t/, $_)[0]}, "\t", "cgST: ", $cgST{(split /\t/, $_)[0]}, "\n" if (-e $st && -e $cgst); 
        print FINAL "$_\t", $meta{(split /\t/, $_)[0]}, "\t", "ST: ", $ST{(split /\t/, $_)[0]}, "\n" if (-e $st && !(-e $cgst) );
        print FINAL "$_\t", $meta{(split /\t/, $_)[0]}, "\t", "cgST: ", $cgST{(split /\t/, $_)[0]}, "\n" if ( !(-e $st) && -e $cgst); 
        print FINAL "$_\t", $meta{(split /\t/, $_)[0]}, "\n" if ( !(-e $st) && !(-e $cgst) );
    }else{
        print FINAL "$_\t", "ST: ", $ST{(split /\t/, $_)[0]}, "\t", "cgST: ", $cgST{(split /\t/, $_)[0]}, "\n" if (-e $st && -e $cgst);
        print FINAL "$_\t", "ST: ", $ST{(split /\t/, $_)[0]}, "\n" if (-e $st && !(-e $cgst) );
        print FINAL "$_\t", "cgST: ", $cgST{(split /\t/, $_)[0]}, "\n" if ( !(-e $st) && -e $cgst); 
        print FINAL "$_\n" if ( !(-e $st) && !(-e $cgst) );
    }
}
