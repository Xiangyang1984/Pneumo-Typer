...
use vars qw($image %hash_nif %color_hash %cluster_color_hash %cluster_color_hash_2 %direction_F_R %shift_distance_x $yellow $green $blue $black $black2 $red $white $gray $dgray $tree_width @array_image %cluster_GeneName_hash @rgb_color);

#create a svg image 

sub create_image_SVG {

    my ($home_directory, $directory_part_TFT, $strain_reorder_file, $phylogenetic_file, $show_tree_topology, $workplace, $ref_options) = @_;
    ...
    my $image_width = $array_image[0]*$options{figure_Scale_up_multiple}/20+$options{right_shift}+$array_image[2]+$tree_width;  # image_height 2019-06-26
...
$max_number =~ s/\n//g; 
color_spectrum ($max_number, 'black', 'yellow'); 
#print "$max_number\n";
    ...
    if (defined $options{phylogenetic_file}) {
        $temp_strain_reorder_file = TreeDraw($workplace, $phylogenetic_file, $options{show_tree_topology}, $options{show_tree_branch}, $options{show_tree_bootstrap}, $options{x_step}, $options{left_shift}, $options{up_shift}, $options{distance_between_two_genomes}, $options{font_family}, $options{strain_name_font_size});  # display the tree
    }
    ...
    if (defined $options{strain_reorder_file} || defined $options{phylogenetic_file}) {
        ...
        foreach my $part_reorder (sort {$a<=>$b} keys %strain_reorder_hash){ 
            ...
            foreach my $tbl_part_file(sort @dir_part_TFT){ 
                ...
                if(($tbl_part_file =~ /$check_value/) and ($tbl_part_file =~ /tbl$/)) { # delete temp file generated when part_TFT file were edited to modify the genename

                    $tbl_part_file =~ s/(.tbl)//g;
...
#print "dddddddddddddddddddddddddd\n";
        foreach my $tbl_part_file(sort @dir_part_TFT){
            ...
            $eachfile++;
#print "ffffffffffffffttttttttttttttt\n";
...
#add antibiotic type
###########################################################################################################
my ($type_name, $type_number);
my $left_number = 0;
foreach(@type_classification){
    chomp;
    ($type_name, $type_number) = split "\t", $_;

    my $head_start = 1+$left_number*50*$options{figure_Scale_up_multiple}/20+$tree_width+15+$array_image[2]+50*$options{figure_Scale_up_multiple}/20*0.1;
    $left_number += $type_number; 
    my $head_end = $left_number*50*$options{figure_Scale_up_multiple}/20+$tree_width+15+$array_image[2]-50*$options{figure_Scale_up_multiple}/20*0.1;
    $image->line($head_start,$options{up_shift}-40,$head_end,$options{up_shift}-40,$red);
    $image->line($head_start,$options{up_shift}-40,$head_start,$options{up_shift}-30,$red);
    $image->line($head_end,$options{up_shift}-40,$head_end,$options{up_shift}-30,$red);
    $image->stringrotate(($head_end+$head_start)/2, $options{up_shift}-40, $options{font_family}, $options{font_style}, $options{label_font_size}, $type_name, $blue, -$options{rotate_gene_label});
...
my $length_count=0;
        foreach(@rgb_color){
            my $start =$options{left_shift}+5+($image_width/20/4)*$length_count;
            my $end = $options{left_shift}+5+($image_width/20/4)*($length_count+1);
...
#print $_, "\t$length_count\n";
            $image->filledPolygon(rec_arrow_F($start, $end, 0, $options{arrow_relative_Height}*3, $image_height-40),$temp_color);
            #$image->polygon(rec_arrow_F($start+750,$end+750,0,10, 200), $temp_color);
            $image->stringrotate(($start+$end)/2, $image_height-40+$options{arrow_relative_Height}*3+$options{label_font_size}, $options{font_family}, $options{font_style}, $options{label_font_size}, $length_count-1, $black, 0);
        }
       $image->stringrotate($options{left_shift}+5, $image_height-40+$options{arrow_relative_Height}*3+$options{label_font_size}*2, $options{font_family}, $options{font_style}, $options{label_font_size}, "Gene number", $black, 0);
###########################################################################################################

    open (SVG_MAP, ">$workplace/heatmap.svg") or die("Cannot open file for writing");
...
sub draw_map_SVG {
    ...
    $image->stringrotate($left_shift+5, $Y_centre+$strain_name_height/4-$strain_name_shift_Y, $font_family, $font_style, $strain_name_font_size, $filename, $given_color_hash{$strain_name_font_color}, 0); # genomic names
#print "ttttt: stringrotate\n";
    ...
    while (<ABL>){
        ...
        $strart=($cds[0]-$min[0])*$figure_Scale_up_multiple/20+$left_shift+15+$array_image[2];  # $left_shift = $tree_width
        $end=($cds[1]-$min[0])*$figure_Scale_up_multiple/20+$left_shift+15+$array_image[2];
        ...
        if ($show_label eq "T") {
           ...
           #$tag =~ s/;.*//g; #delete locus_Tag
            if ($eachfile == 1){
                $image->stringrotate(($end+$strart)/2, $Y_centre-$arrow_high-$gene_label_shift_Y, $font_family, $font_style, $label_font_size, $geneName_change_hash{$tag}, $given_color_hash{$label_font_color}, -$rotate_gene_label); # Default color is dgray for gene label of the other genes
            }
        ...
        if ($end>$strart){                                            
                my $index_rgb_color = $cds[4];
                my @final_rgb = split ",", $rgb_color[$index_rgb_color];
                my $color = $image->colorAllocate($final_rgb[0], $final_rgb[1], $final_rgb[2]);
                
                $image->filledPolygon(rec_arrow_F($strart,$end,$arrow_length,$arrow_high, $Y_centre),$color);#$rgb_color[$cds[4]]  
#print $filename,":\t$rgb_color[$index_rgb_color]\t",$image->colorAllocate($rgb_color[$index_rgb_color]);
                $image->polygon(rec_arrow_F($strart, $end, $arrow_length, $arrow_high, $Y_centre), $color) if $cds[2] eq "CDS"; #set color of border for CDS genes
...
sub color_spectrum {
    ...
    my ($color_num, $color_left, $color_right) = @_;
    $color_num = $color_num+1;
    use Color::Spectrum;
    my $spectrum = Color::Spectrum->new;
    my @color = $spectrum->generate($color_num, $color_left, $color_right);

    #my @rgb_color;
    foreach (@color){
        $_ =~ s/\#//;
        my ($a, $b, $c, $e, $f, $g) = split //, $_;
        #print join("\n", @color);
        my $rgb_number = hex($a.$b).",".hex($c.$e).",".hex($f.$g);
        #my $rgb = $image->colorAllocate(1,1,1);
        #print "rgb_number: $rgb_number\n";
        push @rgb_color, $rgb_number;
    }
