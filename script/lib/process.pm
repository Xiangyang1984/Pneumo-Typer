...
sub cluster_GeneName_hash {
    ...
    while (<CLUSTER>){
        ...
        if (scalar @cluster >= 1) {   #only color homologous gene cluster with genes number >=2  [2023-04-10 added by Xiangyang Li shown all gene including unique genes]
            my @tmp_array;
...
#svg string_width
        ...
        push @max_start_right, $median_value;
        # obtain the key-value of %start_right
        push @start_right, $each_max_width[$#each_max_width]-$each_max_width[0]-$median_value;

        $shift_distance_temp{$each_TFT_file} = $median_value; 
        #print $each_TFT_file,"\t",$each_max_width[$#each_max_width],"\t",$each_max_width[0],"\t",$each_max_width[$#each_max_width]-$each_max_width[0],"\t",$median_value, "\n";  
    }#
         @text_name_width_svg = sort{$b<=>$a} @text_name_width_svg;
         @text_name_width_png = sort{$b<=>$a} @text_name_width_png;

         @max_start_position = sort{$a<=>$b} @max_start_right;
         my $max_start = $max_start_position[$#max_start_position];

         foreach (@start_right) {          
             push (@max_width_right, $max_start+$_);
         }
         ...
         #print $max_start,"\t",$max_width_right[$#max_width_right],"\t",$interested_gene_lengthest,"\n";
...
##################################################################################################
###### Subrounting--figure_size_width_heatmap
###### Function:
###### retrieved the max_left_length and max_rigth_length around the start position of interested gene, and then caclutate the width of figure size
##################################################################################################
sub figure_size_width_heatmap {
...
#png string_width
        my @bounds = GD::Image->stringFT($black_tmp, "$font_family:$font_style", $strain_name_font_size, 0, 10, 10, "$filename");
        my $length_tmp =  $bounds[2]-$bounds[0];
        push (@text_name_width_png, $length_tmp);
        $string_height_png{$filename} = $bounds[1]-$bounds[7];
...
#svg string_width
    ...
    my $length_region = $each_max_width[$#each_max_width] - $each_max_width[0]+1;

    return ($length_region, $text_name_width_png[0], $text_name_width_svg[0], \%string_height_png);
