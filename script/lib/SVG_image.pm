...
sub create_image_SVG {
    ...
    if (defined $options{strain_reorder_file}) { #$strain_reorder_file
        open (STRAIN_REORDER_0, $strain_reorder_file) or die "Must supply strain_reorder_file\n";
        my @HHH = <STRAIN_REORDER_0>;
        @HHH = grep ($_ !~ /^\s/, @HHH);
        $image_height=$options{distance_between_two_genomes}*(scalar @HHH)+$options{up_shift}+$options{down_shift};  # image_height 2023-07-14
    } 
    close STRAIN_REORDER_0;
    ...
    if (defined $options{expand_tab}){
        ...
        foreach (keys %tab_hash){
            ...
            #print "$_\t$expand_shown\t1\t$tab_hash{$_}\n";
        }
    ...
    my $image_width = ($array_image[0]+$array_image[1])*$options{figure_Scale_up_multiple}/20+$options{right_shift}+$array_image[5]+$tree_width+$width_tab_file;  # image_height 2019-06-26
    ...
    if ($options{gene_color_filled} eq  "T") {
        ...
        #subroutine to related color value with each set of homologous gene cluster
    ...
    my $final_place = dirname $workplace;
    open (SVG_MAP, ">$final_place/CPS_operon.svg") or die("Cannot open file for writing");
...
sub draw_map_SVG {
    ...
    $filename =~ s/.tbl(.*)//g;
    #$filename =~ s/_/ /g;
    my $strain_name_height = metrics::string_height_svg("$font_family:$font_style", $strain_name_font_size); #genome name startpopint
    $image->stringrotate($left_shift+2.5, $Y_centre+$strain_name_height/4-$strain_name_shift_Y, $font_family, $font_style, $strain_name_font_size, $filename, $given_color_hash{$strain_name_font_color}, 0); # genomic names $given_color_hash{$strain_name_font_color}
    ...
    if (defined $tab_hash{$filename}){
        ...
        foreach (@tab_text){
            ...
            $image->stringrotate($tree_width+10+$array_image[5]+5+$extand_rigth_distance, $Y_centre+$strain_name_height/4-$strain_name_shift_Y, $font_family, $font_style, $Ename_font_size, $text, $given_color_hash{$Ename_font_color}, 0); # if defined $tab_hash{$filename}; # tab names
            $image->stringrotate($tree_width+10+$array_image[5]+5+$extand_rigth_distance+$max_width/2, $Y_centre-$arrow_high-$gene_label_shift_Y, $font_family, $font_style, $Ename_font_size, $add_head[$ccccnum], $given_color_hash{$Ename_font_color}, -$rotate_gene_label) if $eachfile == 1; # Serotype, ST and cgST
    ...
    while (<ABL>){
        ...
        if ($direction_F_R{$filename_shift} ge 1) {
            $strart=($cds[0]-$min[0]+$shift_distance_x{$filename_shift})*$figure_Scale_up_multiple/20+$left_shift+$array_image[5]+15+$width_tab_file;  #gene startpoint
            $end=($cds[1]-$min[0]+$shift_distance_x{$filename_shift})*$figure_Scale_up_multiple/20+$left_shift+$array_image[5]+15+$width_tab_file;     #gene startpoint

        }

        else {

            $strart=(abs($cds[0]-$min[$#min])+$shift_distance_x{$filename_shift})*$figure_Scale_up_multiple/20+$left_shift+$array_image[5]+15+$width_tab_file; #gene startpoint
            $end=(abs($cds[1]-$min[$#min])+$shift_distance_x{$filename_shift})*$figure_Scale_up_multiple/20+$left_shift+$array_image[5]+15+$width_tab_file;    #gene startpoint
       ...
       # my $right=$Y_centre-1.25*$arrow_high;
        ...
        if (defined ($hash_nif{$tag})) {
            ...
            $image->stringrotate(($end+$strart)/2, $Y_centre-$arrow_high-$gene_label_shift_Y, $font_family, $font_style, $label_font_size, $geneName_change_hash{$tag}, $given_color_hash{$interested_gene_label_font_color}, -$rotate_gene_label); # highligth interested gene
        }
            ...
            if (defined $xx{$node->ancestor}) {
                ...
                if ( $bootstrap eq "T" ) {               
                    if (defined $node->ancestor->id) {
                        my $bootstrap_value =  int (($node->ancestor->id));
