#perl /home/xiangyang/Mazhongrui/Streptococcus_pneumoniae/pneumo-typer-v1.0.2/pneumo-typer血清型预测结果统计工具/serotype_correct_sta.pl /home/xiangyang/Mazhongrui/Streptococcus_pneumoniae/pneumo-typer-v1.0.2/DATABASE/serotype_matrix.txt /home/xiangyang/Mazhongrui/Streptococcus_pneumoniae/pneumo-typer-v1.0.2/pneumo-typer_workplace/result_statistics/Statistics_OUT/CPS_LocusTag_statistics /home/xiangyang/Mazhongrui/Streptococcus_pneumoniae/pneumo-typer-v1.0.2/pneumo-typer血清型预测结果统计工具/Serotype_sta.out

use strict;
use warnings;
open (TU, "/home/xiangyang/Mazhongrui/Streptococcus_pneumoniae/pneumo-typer-v1.0.2/pneumo-typer血清型预测结果统计工具/ture.txt");
my %t;
my %m;
while(<TU>){
    chomp;
    ...
    $m{$key} = $value;
    my $strq = $value;
    my $count = $strq =~ tr/0-9/0-9/;

    if ($count == 1){
        $t{$key} = "0".$value;
    }else{
        $t{$key} = $value if $count > 1;
    }
...
############################################
    ...
    if ($m{$name} !~ /^blank|^N|^n|^0NT|^06E$|^6$|^7$|^24$|^47$|^10X$|^25A\|25F$/){        
        if ($correct_ser eq $t{$name}){
            print OUT "$name\t$correct_ser\t$t{$name}\tRight\n";
        }else{
            if ( ($t{$name} eq "15B|15C") && ($correct_ser =~ /^15B$|^15C$/) ){
                print OUT "$name\t$correct_ser\t$t{$name}\tRight\n";
            }else{
                print OUT "$name\t$correct_ser\t$t{$name}\tWrong\n";
            }
        }
    }
